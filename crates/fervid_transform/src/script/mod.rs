...
pub fn transform_and_record_scripts(
    ...
    if let Some(script_setup) = script_setup {
        ...
        merge_sfc_helper(setup_transform_result.sfc_object, &mut export_obj.props);
...
fn merge_sfc_helper(sfc_object: SfcExportedObjectHelper, dest: &mut Vec<PropOrSpread>) {
    macro_rules! merge {
        ($field: ident, $span: expr, $sym: expr) => {
            if let Some(value) = sfc_object.$field {
    ...
    dest.extend(sfc_object.untyped_fields);
}
