...
use swc_core::{
    common::DUMMY_SP,
    ecma::ast::{ArrayLit, Expr, ExprOrSpread, Ident, ObjectLit},
};
...
impl CodegenContext {
    ...
    pub fn generate_keepalive(&mut self, element_node: &ElementNode) -> Expr {
        ...
        let keepalive_attrs = if element_node.starting_tag.attributes.len() != 0 {
            let mut attrs = Vec::with_capacity(element_node.starting_tag.attributes.len());
            self.generate_attributes(&element_node.starting_tag.attributes, &mut attrs);
...
#[cfg(test)]
mod tests {
    use fervid_core::{AttributeOrBinding, BuiltinType, ElementKind, StartingTag, Node};
