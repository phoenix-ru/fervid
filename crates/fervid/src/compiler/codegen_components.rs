...
use super::{codegen::CodegenContext, codegen_attributes, helper::CodeHelper, imports::VueImports};
...
impl<'a> CodegenContext<'a> {
    pub fn create_component_vnode(
        ...
        let has_attributes_work =
            codegen_attributes::has_attributes_work(starting_tag.attributes.iter());
    ...
    pub fn generate_components_string(&mut self, buf: &mut String) {
        ...
        for (index, (component_name, identifier)) in sorted_components.iter().enumerate() {
            ...
            write!(
                buf,
                "const {} = {}(\"{}\")",
                identifier, resolve_fn_str, component_name
            )
    ...
    fn generate_slots(&mut self, buf: &mut String, children: &[Node], scope_to_use: u32) {
        ...
        for template in &children[..partition_point] {
            ...
            let had_children_work =
                self.generate_element_children(buf, children, false, *template_scope);
        ...
        if default_slot_children.len() > 0 {
            ...
            buf.push_str(self.get_and_add_import_str(VueImports::WithCtx));
            buf.push_str("(() => ");
            self.generate_element_children(buf, default_slot_children, false, scope_to_use);
            CodeHelper::close_paren(buf);
        }
    ...
    fn generate_vmodels<'d>(
        &mut self,
        buf: &mut String,
        directives: impl Iterator<Item = &'d VModelDirective<'d>>,
    ) {
        for (index, directive) in directives.enumerate() {
            ...
            let argument = directive.argument.unwrap_or("modelValue");
