...
const props = defineProps({ foo: String })
    ...
    defineProps<{
      ...
      unknownUnionWithFunction: UnknownType | (() => any)
        ...
        expect(content).toMatch(`object: {
            type: Object,
            required: true
        }`)
        expect(content).toMatch(`objectLiteral: {
            type: Object,
            required: true
        }`)
        ...
        expect(content).toMatch(`functionRef: {
            type: Function,
            required: true
        }`)
        expect(content).toMatch(`objectRef: {
            type: Object,
            required: true
        }`)
        ...
        expect(content).toMatch(`arrayRef: {
            type: Array,
            required: true
        }`)
        expect(content).toMatch(`tuple: {
            type: Array,
            required: true
        }`)
        ...
        expect(content).toMatch(`literal: {
            type: String,
            required: true
        }`)
        ...
        expect(content).toMatch(`recordRef: {
            type: Object,
            required: true
        }`)
        expect(content).toMatch(`interface: {
            type: Object,
            required: true
        }`)
        ...
        expect(content).toMatch(`objectOrFn: {
            type: [
                Object,
                Function
            ],
        ...
        expect(content).toMatch(`nonNull: {
            type: String,
            required: true
        }`)
        expect(content).toMatch(`union: {
            type: [
                String,
                Number
            ],
        ...
        expect(content).toMatch(`literalUnion: {
            type: String,
            required: true
        }`)
        expect(content).toMatch(`literalUnionNumber: {
            type: Number,
            required: true
        }`)
        expect(content).toMatch(`literalUnionMixed: {
            type: [
                String,
                Number,
                Boolean
        ...
        expect(content).toMatch(`intersection: {
            type: Object,
            required: true
        }`)
        expect(content).toMatch(`intersection2: {
            type: String,
            required: true
        }`)
        ...
        expect(content).toMatch(`unknownUnion: {
            type: null,
            required: true
        }`)
        // intersection containing unknown type: narrow to the known types
        expect(content).toMatch(`unknownIntersection: {
            type: Object,
            required: true
        }`)
  ...
  `, { outputSetupBindings: true })
    ...
    `, { outputSetupBindings: true })
        ...
        expect(content).toMatch(`quux: {
            type: Function,
            required: false,
            default () {}
        }`)
        expect(content).toMatch(`quuxx: {
            type: Promise,
            required: false,
            async default () {
        ...
        expect(bindings).toStrictEqual({
            ...
            // Here I assume this is a bug in official compiler because `withDefaults` returns the same
            // reactive object as normal `defineProps`, therefore Fervid produces reactive const.
  ...
  `, { outputSetupBindings: true })
    ...
    describe('errors', () => {
        test('w/ both type and non-type args', () => {
            ...
            expect(errors.length).toBe(1)
            expect(errors[0].message).toMatch('DefinePropsTypeAndNonTypeArguments')
        })
    })
    ...
    defineProps<{
      ...
      'til~de': unknown
        ...
        assertCode(content)
        expect(content).toMatch(`"spa ce": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"exclamation!mark": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`'double"quote': {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"hash#tag": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`dollar$sign: {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"percentage%sign": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"amper&sand": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"single'quote": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"round(brack)ets": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"aste*risk": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"pl+us": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"com,ma": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"do.t": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"sla/sh": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"co:lon": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"semi;colon": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"angle<brack>ets": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"equal=sign": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"question?mark": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"at@sign": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"square[brack]ets": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"back\\\\slash": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"ca^ret": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"back\`tick": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"curly{bra}ces": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"pi|pe": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"til~de": {
            type: null,
            required: true
        }`)
        expect(content).toMatch(`"da-sh": {
            type: null,
            required: true
        }`)
        expect(bindings).toStrictEqual({
    ...
    test('custom element retains the props type & production mode', () => {
      ...
      const props = defineProps<{ foo: number}>()
            ...
            { filename: 'app.ce.vue', isCustomElement: true },
            { isProduction: true, /* customElement: filename => /\.ce\.vue$/.test(filename) */ },
        )
